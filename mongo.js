...
static async signup(data){
      try {
        ...
        const collectionName3 = 'employeehistory';
                  ...
                  const db = await client.db("hotel");
                  const col = db.collection("signuphotel")
                  const doc = { name:data.name, email:data.email,password:data.password,confirm_password:data.confirm_password};
...
static async addroom(data){
  try {
    ...
    const col = db.collection("room")
              ...
              const doc = { roomno:data.roomno, acNonac:data.acNonac,price:data.price,available:data.available ,status:data.status,bed:data.bed};
              ...
              return e; // Handle the error appropriately
          } finally {
...
static async book(data){
  try {
    ...
    const col2 = db.collection("historyofbooking")
              const doc = { custname:data.custname , custidtype:data.custidtype,custidnumber:data.custidnumber,custmobno:data.custmobno ,gender:data.gender,roomNo:data.roomNo,indate:data.indate,intime:data.intime,deposite:data.deposite,price:data.price};
              const existingDoc = await col.findOne({
                $or: [
                  { custidnumber: doc.custidnumber },
                  { mobno: doc.custmobno }
                ]
              });
              if (existingDoc) {
...
static async Addemp(data){
  try {
    // const d=JSON.parse(localStorage.getItem("loggedin_user_nam"));
    console.log("connecting");
    await client.connect();
    ...
    const doc = { empid:data.empid, empname:data.empname,empage:data.empage,empadharno:data.empadharno ,empgender:data.empgender,empmobno:data.empmobno,empjob:data.empjob,empsalary:data.empsalary,working:data.working};
...
static async checkout(data) {
  try {
     ...
     const result= await col3.updateOne(filter2, update2);
      ...
      console.log("dsnbnnnnnnnnnnnnnnnffffffffffff",result)
